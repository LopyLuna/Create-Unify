...
public class UnifyBlocks {
	...
	public static BlockEntry<Block> MetalBlock(String id, String lang, SoundType soundType, MaterialColor color, TagKey<Block> tags, NonNullSupplier<? extends Block> block) {
		return REGISTRATE.block(id + "_block", Block::new)
	...
	public static BlockEntry<Block> RawOreBlock(String id, String lang, SoundType soundType, MaterialColor color, TagKey<Block> tags, NonNullSupplier<? extends Block> block) {
		return REGISTRATE.block("raw_" + id + "_block", Block::new)
				.initialProperties(block)
				.properties(p -> p.color(color).requiresCorrectToolForDrops())
				.properties(p -> p.sound(soundType))
				.transform(pickaxeOnly())
